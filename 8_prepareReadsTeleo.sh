...
names=(`cut -f 1 Teleo_samples_20210621.txt`)
R1=(`cut -f 2 Teleo_samples_20210621.txt`)
R2=(`cut -f 3 Teleo_samples_20210621.txt`)
x=(`wc -l Teleo_samples_20210621.txt`)
y=$((x-1))
z=$((x-2))

echo -n "" > fasta_file.txt
echo -n "" > group.txt
echo -n "" > Teleo_samples_20210621.fasta
...
#convert into fasta 
	...
	if (($i > $z)) ; then # number must be = i-2
	...
	cat ${names[i]}.assembled_retained.fasta >> Teleo_samples_20210621.fasta
...
mv merge.groups Teleo_samples_20210621.groups
rm -f mothur*logfile
